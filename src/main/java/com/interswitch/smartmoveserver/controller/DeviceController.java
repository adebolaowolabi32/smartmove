...
*/
    ...
    private Iterable<Device> getAll() {
    ...
    private Iterable<Device> findAllByOwner(@Validated @PathVariable long owner) {
        return deviceService.findAllByOwner(owner);
    }

    @GetMapping(value = "/findByType/{type}", produces = "application/json")
    private Iterable<Device> find(@Validated @PathVariable Enum.DeviceType type) {
