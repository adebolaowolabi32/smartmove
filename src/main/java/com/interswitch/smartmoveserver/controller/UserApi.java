...
/*    @PostMapping(value = "/{type}/{id}", consumes = "application/json", produces = "application/json")
    ...
    private List<User> findByParent(@Validated @PathVariable long parent) {
   ...
   /* @GetMapping(value = "/{id}", produces = "application/json")
    public List<User> getByType(@Validated @PathVariable long id) {
        ...
        User.Role parentType = user != null ? user.getType() : null;

        if(parentType != null && parentType.equals(User.Role.ISW_ADMIN)){
